...
static void diagnostic_row(ui_context *ui, float label_x, float value_x, float y,
    ...
    ui_text(ui, label_x, y, 0.30f, ui->theme.muted, label);
    ui_text(ui, value_x, y, 0.31f, color, value);
}

void ui_view_diagnostics_top(ui_context *ui, const ui_snapshot *snapshot) {
    ui_header(ui, "Diagnostics", snapshot->build_id);
    ...
    diagnostic_row(ui, 18, 91, 114, "Netmask", snapshot->netmask, ui->theme.text);
    diagnostic_row(ui, 18, 91, 134, "Broadcast", snapshot->broadcast_ip, ui->theme.text);
    diagnostic_row(ui, 18, 91, 154, "Peer", snapshot->static_peer ? snapshot->peer_ip : "automatic", ui->theme.text);
    diagnostic_row(ui, 18, 91, 174, "Config", snapshot->external_config ? "SD override" : "ROMFS", ui->theme.text);
    char value[32];
    snprintf(value, sizeof(value), "%u", snapshot->probe_port);
    diagnostic_row(ui, 18, 91, 194, "Probe port", value, ui->theme.text);
    ...
    diagnostic_row(ui, 214, 310, 194, "Mcast IF", value, ui->theme.warning);
}

void ui_view_diagnostics_bottom(ui_context *ui, const ui_snapshot *snapshot) {
    ui_text(ui, 8, 41, 0.42f, ui->theme.text, "Probe and endpoint detail");
    ...
    diagnostic_row(ui, 18, 126, 78, "Probe packets", value, ui->theme.text);
    diagnostic_row(ui, 18, 126, 100, "Last sender", snapshot->last_probe_sender, ui->theme.text);
    snprintf(value, sizeof(value), "%.26s", snapshot->last_probe_payload);
    diagnostic_row(ui, 18, 126, 122, "Last payload", value, ui->theme.text);
    snprintf(value, sizeof(value), "%ld / %ld", (long)snapshot->writer_qos_policy,
             (long)snapshot->reader_qos_policy);
    diagnostic_row(ui, 18, 126, 144, "QoS policy", value, ui->theme.text);
    ...
    diagnostic_row(ui, 18, 126, 166, "Bind/join/loop", value,
                   snapshot->probe_bind_error || snapshot->probe_membership_error
                       ? ui->theme.warning : ui->theme.text);
    ui_text(ui, 8, 217, 0.30f, ui->theme.muted, "Detailed state is isolated from the main dashboard");
}